...
$current_dir = empty($_GET['dir']) ? null : $_GET['dir'];

$search_query = isset($_GET['search']) ? trim($_GET['search']) : '';
$filter_type = $_GET['file_type'] ?? 'All Files';
$sort_strategy = $_GET['sort'] ?? 'Newest Created';
$show_starred = isset($_GET['starred']) && $_GET['starred'] == 'true' ? 1 : 0;
...
if (empty($search_query) && empty($show_starred) && $filter_type == 'All Files') {
    if ($current_dir === null) {
...
if (!empty($current_dir)) {
    ...
    while (!empty($temp_dir)) {
        $stmt_bc = $koneksi->prepare("SELECT id, name, parent_id FROM files WHERE id = ? AND user_id = ? AND type = 'folder'");
        $stmt_bc->bind_param("ss", $temp_dir, $user_id);
        $stmt_bc->execute();
        $result_bc = $stmt_bc->get_result();
        if ($row_bc = $result_bc->fetch_assoc()) {
            array_unshift($breadcrumb, ['id' => $row_bc['id'], 'name' => $row_bc['name']]);
            $temp_dir = $row_bc['parent_id'];
        } else {
            break;
        }
        $stmt_bc->close();
        ...
        <ol class="breadcrumb mb-0">
            ...
            <?php foreach ($breadcrumb as $idx => $crumb): ?>
                <?php if ($idx === count($breadcrumb) - 1): ?>
                    <li class="breadcrumb-item active text-dark fw-medium" aria-current="page"><?= htmlspecialchars($crumb['name']); ?></li>
                <?php else: ?>
                    <li class="breadcrumb-item">
                        <a href="index.php?dir=<?= $crumb['id']; ?>" class="text-decoration-none text-primary fw-medium"><?= htmlspecialchars($crumb['name']); ?></a>
                    </li>
                <?php endif; ?>
            <?php endforeach; ?>
        </ol>
        ...
        <div class="placeholder-container-box border border-dashed rounded d-flex align-items-center justify-content-center p-5 mt-3">
            <div class="text-center py-5">
                <div class="icon-cloud-display-wrapper mb-3 mx-auto">
                    <i class="bi bi-hdd-network text-muted opacity-50 display-4"></i>
                </div>
                <h6 class="fw-medium text-dark mb-1">This folder is empty</h6>
                <p class="text-muted small mx-auto max-width-p">Drag and drop files here or use the New button to upload files or create folders.</p>
            </div>
        </div>
    <?php else: ?>
        <?php 
        ...
        <?php if (!empty($folders)): ?>
            ...
            <div class="grid-layout-folder mb-5">
                <?php foreach ($folders as $folder): 
                    $is_shared = !empty($folder['share_token']);
                ?>
                    <div class="drive-item folder-card d-flex align-items-center p-2 gap-2 shadow-sm position-relative" 
                        ...
                        <div class="d-flex align-items-center gap-1">
                            <?php if ($is_shared): ?>
                                <i class="bi bi-share-fill text-primary small me-1" title="Dibagikan"></i>
                            <?php endif; ?>
