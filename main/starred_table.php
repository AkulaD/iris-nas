...
$sql = "SELECT * FROM `files` WHERE `user_id` = ? AND `deleted_at` IS NULL AND `is_starred` = 1";
$params = [$user_id];
$types = "s";
...
<div id="custom-context-menu" class="dropdown-menu shadow-sm context-menu-box">
    ...
    <a class="dropdown-item d-flex align-items-center gap-2 py-2" href="#" data-action="rename"><i class="bi bi-cursor-text text-muted"></i> Ganti Nama</a>
    ...
    <nav aria-label="breadcrumb" class="mb-4">
        <ol class="breadcrumb mb-0">
            ...
            <li class="breadcrumb-item active text-dark fw-medium" aria-current="page">
                <i class="bi bi-star-fill text-warning me-1"></i> Berbintang
            </li>
        </ol>
    </nav>
        ...
        <div class="placeholder-container-box border border-dashed rounded d-flex align-items-center justify-content-center p-5 mt-3">
            <div class="text-center py-5">
                <div class="icon-cloud-display-wrapper mb-3 mx-auto">
                    <i class="bi bi-star text-muted opacity-50 display-4"></i>
                </div>
                <h6 class="fw-medium text-dark mb-1">Tidak ada file berbintang</h6>
                <p class="text-muted small mx-auto max-width-p">Tambahkan bintang pada file atau folder agar mudah ditemukan di sini.</p>
            </div>
        </div>
    <?php else: ?>
        <?php 
...
<div class="modal fade" id="shareModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-disabled="modal" data-bs-dismiss="modal">Tutup</button>
            </div>
        </div>
    </div>
</div>